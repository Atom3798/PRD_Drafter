...
One approved change has been made since: `document_format`, `detail_level` and
`document_audience` were added to `PrdInputs`, along with `FORMAT_SECTIONS`.
Format is not cosmetic — it selects *which* sections get generated, so a lean
one-pager produces 8 sections rather than all 21. `test_contract_sync.py`
compares the presets on both sides.
...
| 5 | Seven-step wizard with autosave | Done |
